...
export class TranscendRuntime {
  ...
  async _executeWithRetries(spec, inputs) {
    ...
    do {
      ...
      if (spec.execution.note?.includes('stdin') ||
        ...
        // If inputs.input is provided as a string, pipe it directly (e.g. for json_query)
        if (typeof inputs.input === 'string' && inputs.input.length > 0) {
          stdinData = inputs.input;
        }
      }
  ...
  async _executeFallback(spec, inputs) {
    ...
    // Handle json_query fallback with input string
    if (spec.name === 'json_query' && typeof inputs.input === 'string' && inputs.input.length > 0) {
      stdinData = inputs.input;
    }
